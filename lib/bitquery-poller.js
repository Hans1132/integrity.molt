...
async function pollBitquery() {
  ...
  const sinceIso = new Date(sinceMs + 1).toISOString(); // +1ms: Bitquery `since` is inclusive (>=)
  ...
  try {
    ...
    stmtRecordError.run(err.message.slice(0, 500), startTs, DEX_ID);
