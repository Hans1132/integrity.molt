...
function recordLiquidityEvent(eventType, poolAddress, mint, amount, timestamp, txHash) {
  ...
  if (!Number.isFinite(amount) && amount != null) {
    console.warn(`[liquidity-processor] Invalid amount ${amount} for ${eventType} — using 0`);
  }

  switch (eventType) {
    case 'ADD_LIQUIDITY':
      stmtAddLiquidity.run(poolAddress, mint, validAmount, timestamp, timestamp);
      break;

    case 'REMOVE_LIQUIDITY':
      stmtRemoveLiquidity.run(poolAddress, mint, validAmount, timestamp, timestamp, timestamp);
      break;
