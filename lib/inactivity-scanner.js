...
// V4_START_TS: timestamp when Bitquery pipeline was initialized.
// Scanner only flags pool_activity rows created by V4 (created_at >= V4 start),
// avoiding retroactive mislabelling of any V3 Helius-era rows.
const stmtGetV4Start = db.prepare(`
...
function scanForInactivity() {
  ...
  try {
    ...
    return { poolsMarkedInactive: 0, newRugPullsFlagged: 0, error: err.message, timestamp: now };
  }
}
