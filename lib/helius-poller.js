...
// types: Helius event types to request. Raydium AMM v4 uses standard AMM events;
// other DEXes use custom instruction types Helius classifies as SWAP or UNKNOWN.
// Fetching ADD_LIQUIDITY/REMOVE_LIQUIDITY for non-Raydium DEXes produces only
// empty-window 404s (~50 wasted credits/cycle). Trade-off: last_liquidity_remove_ts
// stays NULL for those 4 DEXes — inactivity scanner won't trigger on them.
const DEX_PROGRAMS = [
  { id: 'raydium_amm',    name: 'Raydium AMM v4',  programId: '675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8',
    types: ['ADD_LIQUIDITY', 'REMOVE_LIQUIDITY', 'WITHDRAW', 'SWAP'] },
...
async function pollSingleDex(dex) {
  ...
  try {
    while (totalPages < MAX_PAGES) {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          // Helius returns 404 with continuation token when no typed events exist
          // in the search window (window may contain only UNKNOWN-type txs).
          // Body: {"error":"...query the API again with the `before-signature` parameter set to <sig>"}
          const contMatch = body.match(/before-signature[`'\s]*\s*parameter\s+set\s+to\s+([A-Za-z0-9]{40,100})/);
          if (contMatch) {
            console.log(`[helius-poller] ${dex.name}: empty window (404), continuing from ${contMatch[1].slice(0, 16)}…`);
            pageBefore = contMatch[1];
            continue;
          }
          console.warn(`[helius-poller] DEX ${dex.name} returned 404 (no continuation), skipping`);
    ...
    if (totalPages >= MAX_PAGES && !reachedKnownTx) {
      console.warn(`[helius-poller] ${dex.name}: MAX_PAGES (${MAX_PAGES}) reached — possible gap, not fully caught up`);
    }
