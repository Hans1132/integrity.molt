...
| `token_audit` | $0.75 | Mid-tier SPL token launch audit. Mint authority, freeze authority, holder distribution, liquidity analysis, rug risk assessment. | `address` (text/plain) | Audit result `{risk_score, category, summary, mint_info, concentration, treasury_analysis, findings, signature, ...}` | `POST /scan/token` | Yes | 60 min |
